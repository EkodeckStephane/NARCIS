...
The learned representation, projection-selection rule, codebook, complementary group banks, session keys, payloads, and holdout label predictions are held fixed while RS parity is evaluated at 0, 32, 64, 96, and 128 bytes. The coded payload is rescheduled at each level because FEC expansion changes the required 3-bit symbol demand. Communication cost is the number of unchanged natural images transmitted; since each coded symbol consumes a five-cover group, an $m$-group session sends $5m$ images. The same 1,200 partition--session--attack trials are reused at every parity level. Exact Clopper--Pearson intervals quantify each recovery proportion, and exact paired McNemar tests compare the four adjacent parity levels together with the RS0--RS128 endpoint contrast; Holm adjustment controls multiplicity across these five prespecified comparisons.

\subsection{Selection-detectability audit}
The detector audit reconstructs the complete 30-session selected-cover schedule for every external partition. For each partition and each of 20 repetitions, a 65/35 image-disjoint train/test split is created and reused across SRM-lite residual features with ExtraTrees, GLCM texture features with logistic regression, and a 30-head CNN. AUC is macro-averaged across the 30 session heads within each split. Statistical reporting is hierarchical: the 20 repetitions characterize within-partition split variability, whereas the five external partition means form the replication level for between-partition summaries and confidence intervals. This separation prevents overlapping splits from being treated as independent external replications.
