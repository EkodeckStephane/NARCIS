...
\method and Guo--Ping address closely related natural-image selection problems with different recovery events. Guo--Ping construct a PZM/SA-PQE feature space, cluster the resulting binary vectors, and select one stability-regularized representative per message segment~\cite{guo2026capacity}. Their native robustness metric is the fraction of CID representatives whose own message segment is recovered after attack, whereas ARCIS counts success only when the complete plaintext is recovered after majority decoding, RS correction, framing, and AES-GCM verification. Guo--Ping define one representative $Z_k$ per cluster and map each message segment directly to that representative; the source protocol does not impose ARCIS's within-session no-cover-reuse requirement. ARCIS therefore treats multiplicity availability for the complete protected demand as an explicit finite-index feasibility constraint rather than inferring feasibility from the existence of one representative per symbol.
