...
Final steganalysis & 3 detectors; 5 external partitions $\times$ 20 splits & Partition-level mean AUC 0.504, 0.513, 0.527; hierarchical $n=5$ inference & Selection-discrimination characterization\\
...
The partition-level detector analysis separates effect magnitude from replication structure. SRM-lite produces the smallest mean discrimination above chance ($\Delta\mathrm{AUC}=0.0040$), GLCM increases the separation to 0.0128, and the CNN reaches 0.0266. Across the five external partitions, all three mean-AUC intervals remain above 0.5 after treating partitions---rather than overlapping train/test splits---as the replication unit. The magnitude nevertheless remains low, with the CNN partition means confined to 0.5232--0.5301.
