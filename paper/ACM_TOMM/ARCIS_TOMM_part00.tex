...
\begin{abstract}
Selection-based coverless image communication preserves carrier pixels while shifting the design challenge toward reliable symbol recovery, finite-index availability, communication cost, and the statistical properties of the selected image stream. This work introduces \method (Authenticated Robust Cover-Selection Image Signaling) to provide an authenticated finite-index signaling protocol whose feasibility, robustness, coding overhead, and selection detectability can be evaluated within one reproducible framework. \method combines a learned normalized image descriptor, a calibration-locked $K=8$ quantile codebook, complementary five-cover group banks, a keyed cyclic mapping with Gray-indexed symbol order, AES-GCM authenticated framing, replay control, Reed--Solomon (RS) coding, and explicit finite-index feasibility analysis. The operating point was established on BOSSBase 1.01 and then evaluated on the external Caltech-101 corpus across five seeded train--index resamplings using twelve calibration transformations, eight held-out attack parameterizations, a five-level RS-parity ablation, finite-index blocking-risk analysis, and residual, texture-based, and CNN selection detectors. External calibration achieved 1,800/1,800 authenticated recoveries, while unseen holdouts achieved 1,163/1,200 recoveries (96.92\%), with all 37 failures concentrated in the 12\% central-crop condition. A canonical component ablation attributes the principal recovery gain to complementary group construction: 1,163/1,200 recoveries for full \method versus 1,042/1,200 with random groups and 1,038/1,200 with matched bucket selection, while the cyclic mapping is supported by its balancing effect rather than a recovery advantage. A separate control-plane audit completed 150/150 metadata roundtrips and rejected all tested tamper, replay, and wrong-receiver cases (5/5 each). Recovery increased from 829/1,200 (69.08\%) at RS0 to 1,163/1,200 at RS128, while mean transmitted images per session increased from 970 to 2,676.7. At RS128, 8-, 32-, and 64-byte payloads required 2,320, 2,640, and 3,070 images, respectively. Mean macro-AUC values across the five seeded Caltech resamplings were 0.504, 0.513, and 0.527 for SRM-lite, GLCM, and CNN detectors, with resampling ranges 0.501--0.505, 0.512--0.514, and 0.523--0.530. These results establish a reproducible finite-index authenticated signaling construction and quantify the principal tradeoffs among robustness, communication cost, index feasibility, and selection discrimination under the evaluated image-channel conditions.
\end{abstract}
...
Image steganography traditionally hides information by modifying a carrier, whereas coverless approaches communicate through selection, indexing, semantic mapping, or generation. Selection preserves the original pixels, shifting the principal security question toward the distribution of transmitted images and the reliability of their labels after channel perturbations. Recent surveys accordingly separate cover-selection design from detectability and emphasize robustness, capacity, security, practicality, and selected-carrier distribution as distinct evaluation axes~\cite{azam2025review,xiang2025survey,kombrink2025survey}.
