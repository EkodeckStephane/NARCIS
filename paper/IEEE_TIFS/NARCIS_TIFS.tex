...
\author{\emph{St\'{e}phane Ga\"{e}l R.}~Ekodeck,
        ...
        and~\emph{Julius Marcellin}~Nkenlifack
...
% ---------------------------------------------------------------
\begin{abstract}
Selection-based coverless communication requires verifying that a
finite image index can satisfy every message symbol \emph{before}
transmission, not merely that a descriptor achieves high average
robustness. This paper presents \method~(Neural Adaptive Robust
Coverless Image Signaling), an end-to-end protocol that transmits
unchanged natural images while providing authenticated payload
protection, replay rejection, and forward error correction. A compact
...
balanced symbol classes. A multi-attack qualification stage retains
only covers whose labels remain stable under all calibration
conditions. Payloads are protected with AES-GCM; symbols are assigned
through a keyed Gray permutation; Reed--Solomon coding corrects
residual label errors.

Evaluation uses five deterministic partitions each of BOSSBase-1.01
(grayscale, $512\times512$) and Caltech-101 (variable-resolution RGB).
Across 1{,}575 complete-message trials under clean, calibration, and
holdout conditions, \method recovered every authenticated plaintext.
Mean symbol accuracies were 98.74\% and 98.59\%. Net throughput at
$K=16$ scales from 0.184 bits per cover for 8-byte payloads to 1.213
bits per cover for 128-byte payloads. Removing attack qualification on
BOSSBase seed~11 reduced complete-message success from 210/210 to
142/210~(67.6\%), while a calibration-locked projection recovered
210/210. Selection bias measured with three detector classes yielded
AUCs near chance on BOSSBase; Caltech-101 exhibited a
partition-specific CNN signal (mean~AUC~0.533, 95\%~CI~[0.517,
0.548] over 20 repeated fits on the most sensitive partition).
\end{abstract}
...
\textbf{Contribution~2---End-to-end cryptographic and coding
integration.} AES-GCM~\cite{nist2007gcm} authenticates payload and
session metadata with associated sender--receiver identities; a keyed
Gray permutation limits public symbol-to-label correspondence;
monotonic sequence checking rejects replay; Reed--Solomon
coding~\cite{reed1960polynomial} corrects residual label errors. Net
throughput at $K{=}16$ scales from 0.184 to 1.213 bits per cover for
8 to 128-byte payloads~(Section~\ref{subsec:scalability}).

\textbf{Contribution~3---Systematic detectability measurement.} Three
detector classes (global logistic regression, residual feature ensemble,
and a selection CNN) are applied to five partitions of each dataset. On
BOSSBase all confidence intervals include chance performance. Caltech-101
shows a partition-specific CNN signal (mean~AUC~0.533, 95\%~CI
[0.517,~0.548] over 20 repeated fits on the most sensitive partition),
reported transparently rather than suppressed~(Section~\ref{subsec:detect}).
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/Fig_04.pdf}
...
(bytes) & (bytes) & $\times$128-B parity & (bytes) & required &\\
\midrule
8  & 36 & 1 $\times$ 128 & 174 & 348  & 0.184\\
32 & 60 & 1 $\times$ 128 & 198 & 396  & 0.646\\
128 & 156 & 2 $\times$ 128 & 422 & 844 & 1.213\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/Fig_05.pdf}
...
On BOSSBase, all three detector confidence intervals include 0.5.
On Caltech-101, the two classical detectors identify a small
dataset-specific selection shift (intervals exclude 0.5). The CNN
interval is wider and includes 0.5 over five partitions.
...
\begin{tabular}{lllr}
\toprule
Direction & Expl.\ variance & Stable images & Min.\ bucket\\
\midrule
...
\begin{table}[t]
\centering
\caption{BOSSBase seed-11 matched ablation: complete-message success
  over 10 messages per condition (21 conditions, 210 trials total).
  ``Qualif.'' = pc1-qualified (\method); ``No qualif.'' =
  pc1-no-qualification.}
\label{tab:ablation_boss}
\small
\begin{tabular}{lllll}
\toprule
Condition & Qualif. & No qualif. & Symbol acc.\ (no qual.)\\
\midrule
Clean & 10/10 & 10/10 & 1.000\\
JPEG 80, 50 & 10/10 & 10/10 & $>$0.993\\
Gaussian 5, 12 & 10/10 & 10/10 & 0.926--0.930\\
Gaussian 9 (H), 15 (H) & 10/10 & 10/10 & 0.925--0.926\\
