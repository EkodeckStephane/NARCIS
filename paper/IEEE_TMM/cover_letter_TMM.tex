...
\item \textbf{Self-supervised encoder and quantile codebook
  (Contribution~2).} A four-block convolutional network trained with a
  variance--invariance--covariance objective produces 64-dimensional
  normalised descriptors. A quantile partition of the first principal
  component yields balanced symbol classes. Attack qualification
  (12 calibration transforms) removes geometrically unstable images
  before indexing. A calibration-locked random projection search
  recovers 210/210 complete-message trials while increasing the minimum
  stable bucket from 92 to~161 (BOSSBase seed~11). Net throughput at a
  16-symbol operating point scales from 0.184 to 1.213~bits per cover
  for 8- to 128-byte payloads.
...
cross-dataset, multi-detector detectability evidence. The paper
explicitly positions its measured capacity against prior reported values
without overstating cross-protocol comparisons.

\clhead{Author declarations.}
\begin{itemize}
\item The manuscript is original, has not been published, and is not
  under consideration elsewhere. A previous version was desk-rejected
  by \textit{Journal of Information Security and Applications}
  (Ms.\ No.\ JISAS-D-26-03622) for scope reasons and has since been
  substantially revised; this submission is not a transfer.
