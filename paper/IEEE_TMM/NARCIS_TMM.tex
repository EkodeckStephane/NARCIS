...
\textbf{Contribution~2: End-to-end coverless pipeline with three
problem-specific design decisions.}
(a)~\emph{Channel-matched qualification policy}: the 12 calibration
transforms are not merely training augmentations — they define the
qualification criterion itself. A cover is indexed only when its
descriptor label is stable under every one of the 12 transforms,
tying the training invariance objective directly to the indexing
decision.
(b)~\emph{Calibration-locked projection search}: rather than defaulting
to the principal descriptor direction, 32 candidate projections are
evaluated on calibration data and the direction maximising the minimum
qualified-bucket occupancy is selected. This search raised the minimum
stable bucket from~92 (PC1) to~161 while maintaining 210/210 complete
recoveries on BOSSBase seed~11.
(c)~\emph{Full authenticated protocol}: a variance--invariance--covariance
encoder maps images to 64-dimensional descriptors; AES-GCM~\cite{nist2007gcm}
authenticates payloads and session metadata; a keyed Gray permutation
limits symbol-to-label exposure; Reed-Solomon
coding~\cite{reed1960polynomial} corrects residual label errors.
Net throughput at $K{=}16$ scales from 0.184 to 1.213~bits per cover
for 8- to 128-byte payloads~(Section~\ref{subsec:scalability}).
...
and high average robustness on larger datasets. Two distinct capacity
metrics must be distinguished: \emph{nominal symbol capacity}
($\log_2 K$ bits per cover, independent of protocol overhead) and
\emph{net authenticated throughput} (plaintext bits per cover after
AES-GCM, Reed-Solomon parity, and framing overhead). Methods reporting
10--15 bpc measure the former without authentication cost. \method
reports both: 3--4 bpc nominal at $K{=}8$--$16$, and 0.184--1.213 bpc
net for 8- to 128-byte payloads. The two metrics are not
interchangeable; the supporting claim is protocol completeness
with a verified feasible operating point, not maximum nominal capacity.

\begin{table*}[t]
\centering
\caption{Positioning of \method against related work. Values from
  different publications are not treated as a common-protocol ranking.
  $^\dagger$Nominal symbol capacity $= \log_2 K$ bpc, before protocol
  overhead; not comparable to net authenticated throughput.}
\label{tab:position}
\small
...
Nominal capacity$^\dagger$ & High (single-cover) &
High (block/category) &
10--15 bpc on Holidays, VOC, ImageNet &
3--4 bpc ($K{=}8$--$16$) on BOSSBase; 4 bpc ($K{=}16$) on Caltech-101\\
Net auth.\ throughput & Not applicable & Not applicable & Not reported &
0.184--1.213 bpc (8--128 byte payloads at $K{=}16$)\\
...
Error correction & Not reported & Not reported &
Not stated & Reed-Solomon; worst case 63 corrections\\
Authenticated payload & Not reported & Not reported &
...
Two capacity metrics circulate in the coverless steganography
literature and must not be conflated. \emph{Nominal symbol capacity}
($\log_2 K$ bits per cover) measures how many bits a single cover image
encodes at the symbol level, independently of any protocol overhead.
\emph{Net authenticated throughput} measures the plaintext bits
recovered per cover after AES-GCM encryption, Reed-Solomon parity
bytes, framing, and session metadata are accounted for. \method reports
4 bpc nominal at $K{=}16$ and 0.184--1.213 bpc net for 8- to 128-byte
payloads. Systems that report 10--15 bpc measure nominal capacity
without authentication overhead; adding a comparable protocol stack
would reduce their net throughput by the same constant overhead.
Comparing net throughput figures across systems with different protocol
stacks is not meaningful; the appropriate comparison is at the nominal
level, where \method's 4 bpc is lower than Guo-Ping's 10--15 bpc but
is the maximum $K$ satisfying the feasibility constraint on the tested
index sizes.
